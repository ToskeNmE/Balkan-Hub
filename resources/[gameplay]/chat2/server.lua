...
if Config.EnableStaffOnlyCommand then
	RegisterCommand(Config.StaffOnlyCommand, function(source, args)
		local xPlayer = ESX.GetPlayerFromId(source)
		if xPlayer then
			if isAdmin(xPlayer) then
				if xPlayer.proveriDuznost() == true then
					...
					for i=1, #xPlayers, 1 do
						local staff = ESX.GetPlayerFromId(xPlayers[i])
						if staff then
							if isAdmin(staff) and staff.proveriDuznost() then
								TriggerClientEvent('chat:addMessage', staff.source, {
									...
									args = { GetPlayerName(source), table.concat(args, " "), os.date(Config.DateFormat) }
								})
							end
						end
					end
				else
					TriggerClientEvent('dark:client:notify', source, "Nisi na duznosti", 2)
				end
			else
				TriggerClientEvent('dark:client:notify', source, "Nemad dozvolu za ovo", 2)
			end
		end
	end)
end
...
function isAdmin(xPlayer)
	if Config.Grupe[xPlayer.getGroup()] then 
		return true 
	end
	return false
end
