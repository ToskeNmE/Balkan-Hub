...
Citizen.CreateThread(function()
	...
	TriggerServerEvent('hub_markerigrandson:checkIfSentenced')
end)
...
RegisterNetEvent('hub_markerigrandson:inCommunityService')
AddEventHandler('hub_markerigrandson:inCommunityService', function(actions_remaining)
	...
	FillActionTable()
	print(":: Igrac osudjen na: " .. #availableActions)
	...
	while actionsRemaining > 0 and communityServiceFinished ~= true do
		...
		if GetDistanceBetweenCoords(GetEntityCoords(playerPed), Config.ServiceLocation.x, Config.ServiceLocation.y, Config.ServiceLocation.z) > 90 then
			ESX.Game.Teleport(playerPed, Config.ServiceLocation)
				TriggerEvent('chat:addMessage', { args = {_U('escape_attempt') }, color = { 0, 128, 255 } })
				TriggerServerEvent('hub_markerigrandson:extendService')
				actionsRemaining = actionsRemaining + Config.ServiceExtensionOnEscape
		end

	end

	TriggerServerEvent('hub_markerigrandson:finishCommunityService', -1)
	Wait(1)
...
RegisterNetEvent('hub_markerigrandson:finishCommunityService')
AddEventHandler('hub_markerigrandson:finishCommunityService', function(source)
			...
			for i = 1, #availableActions do
				...
				if distance < 1.5 then
					...
					if(IsControlJustReleased(1, 38))then
						...
						TriggerServerEvent('hub_markerigrandson:completeService')
...
function RemoveAction(action)
	...
	if action_pos ~= -1 then
		...
		print("Korisnik je pokusao ukloniti nedostupnu radnju")
	end
