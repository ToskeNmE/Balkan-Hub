...
function kuce(name, message)
    ...
    local poruka = {
      ...
      PerformHttpRequest("", function(err, text, headers) end, 'POST', json.encode({username = "Dark Kuce 📜", embeds = poruka, avatar_url = ""}), { ['Content-Type'] = 'application/json' })
  end
...
AddEventHandler('onServerResourceStart', function(resourceName)
    if resourceName == GetCurrentResourceName() then
        for k, v in pairs(Config.Houses) do
            exports.ox_inventory:RegisterStash('kuca'..tostring(k), 'Sef Kuce - ' .. tostring(k), 50, 50000) 
        end    
    end
end)
            ...
            MySQL.Async.fetchAll("SELECT * FROM bought_houses WHERE houseid=@houseid", {['@houseid'] = id}, function(result)
                ...
                if not result[1] then
                    if xPlayer.getAccount('bank').money >= Config.Houses[id]['price'] then
                        ...
                        kuce("**Kuce**", GetPlayerName(source) .. ' je kupio kucu #'..id.. ' za ' .. Config.Houses[id]['price'].. '$')
                    else
...
AddEventHandler('loaf_housing:sellHouse', function()
    ...
    MySQL.Async.fetchAll("SELECT * FROM users WHERE identifier = @identifier", {['@identifier'] = xPlayer.identifier}, function(result)
        ...
        if Config.Houses[house.houseId]['price'] then
            ...
            exports.ox_inventory:ClearInventory(tostring(house.houseId))
            kuce("**Kuce**", GetPlayerName(source) .. ' je prodao kucu #'..house.houseId.. ' za ' .. (Config.Houses[house.houseId]['price'] * (Config.SellPercentage/100)) .. '$')
        end
    end)
    Wait(1500)
    TriggerClientEvent('loaf_housing:reloadHouses', -1)
