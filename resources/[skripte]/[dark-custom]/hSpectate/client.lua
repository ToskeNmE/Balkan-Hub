-- base
local open_menu_key = 212
-- menu
local nplayers = 0
local player_ids = {}
local player_names = {}
Citizen.CreateThread(function()
	CreateWarMenu('xspectate', 'HUB', 'Spectate Menu', {0.7, 0.2}, 1.0)
	while true do 
		Citizen.Wait(0)
		...
		if WarMenu.IsMenuOpened('xspectate') then
			if WarMenu.Button('~o~Ime','~o~ID') then
...
RegisterNetEvent('xSpectate:menu2')
AddEventHandler('xSpectate:menu2', function()
	if WarMenu.IsMenuOpened('xspectate') then
		WarMenu.CloseMenu()
	else
		TriggerServerEvent('xSpectateMenu')
