...
AddEventHandler('flux_garages:removeCarFromParking', function(plate, networkid)
	...
	if plate ~= nil then
		...
		TriggerClientEvent("hub_notifikacije:obavestenje", xPlayer.source, _U('veh_released'), 5000)
...
ESX.RegisterServerCallback('flux_garages:towVehicle', function(source, cb, plate)
	...
	local identifier = xPlayer.identifier
	MySQL.query("SELECT vehicleid FROM owned_vehicles WHERE owner=@identifier AND plate = @plate",
	{
		['@identifier'] = identifier,
		['@plate'] = plate
	},
...
ESX.RegisterServerCallback('flux_garages:getVehiclesToTow',function(source, cb)
	...
	local identifier = xPlayer.identifier
	local vehicles = {}
	MySQL.query("SELECT * FROM owned_vehicles WHERE owner=@identifier AND state=0",
	{
		['@identifier'] = identifier
	},
	function(data)
...
ESX.RegisterServerCallback('flux_garages:getTakedVehicles', function(source, cb)
	local vehicles = {}
	MySQL.query("SELECT * FROM owned_vehicles WHERE state=2",
	{},
	function(data)
		for _,v in pairs(data) do
			local vehicle = json.decode(v.vehicle)
...
RegisterServerEvent('parezaneregistrovano')
AddEventHandler('parezaneregistrovano', function()
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	TriggerClientEvent("hub_notifikacije:obavestenje", source, "Posto ti vozilo nije <br> registrovano skinuto <br> ti je $2000", 5000)
	TriggerClientEvent("hub_savet:posalji", source, "Valet servis je parkirao vozilo <br> negde u garazi, pronadji ga", 8000)
	--end)
end)
					...
					MySQL.query(
						...
						function(count)
							if #count >= Config.MaxSubs then
								TriggerClientEvent("hub_notifikacije:obavestenje", xPlayer.source, _U('max_subs'), 5000)
							else
								MySQL.Sync.execute(
								...
								TriggerClientEvent("hub_notifikacije:obavestenje", xPlayer.source, _U('sub_added'), 5000)
								TriggerClientEvent("hub_notifikacije:obavestenje", xPlayer.source, _U('you_are_sub', plate), 5000)
							end
						end
					)
				else
					TriggerClientEvent("hub_notifikacije:obavestenje", xPlayer.source, _U('not_owner'), 5000)
				end
			else
				TriggerClientEvent("hub_notifikacije:obavestenje", xPlayer.source, _U('not_veh'), 5000)
			end
		end
	)
end)
...
AddEventHandler('flux_garages:deleteSubowner', function(plate, identifier)
	...
	TriggerClientEvent("hub_notifikacije:obavestenje", xPlayer.source, _U('sub_deleted'), 5000)
end)
