        ...
        if item >= 1 then
            if not UTK.Banks[bank].onaction == true then
                if (os.time() - UTK.cooldown) > UTK.Banks[bank].lastrobbed then
                    ...
                    TriggerClientEvent("utk_fh:outcome", _source, false, "Ova banka je opljackana. Moras da cekas "..math.floor((UTK.cooldown - (os.time() - UTK.Banks[bank].lastrobbed)) / 60)..":"..math.fmod((UTK.cooldown - (os.time() - UTK.Banks[bank].lastrobbed)), 60))
                end
            else
                TriggerClientEvent("utk_fh:outcome", _source, false, "Ova bank se pljacka.")
            end
        else
            TriggerClientEvent("utk_fh:outcome", _source, false, "Nemas krticu.")
        end
    else
        TriggerClientEvent("utk_fh:outcome", _source, false, "Nema dovoljno policajaca.")
    end
end)
...
ESX.RegisterServerCallback('utk_fh:rewardCash', function(source, cb)
