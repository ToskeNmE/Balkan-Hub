...
Citizen.CreateThread(function() while true do local enabled = false Citizen.Wait(1) if disableinput then enabled = true DisableControl() end if not enabled then Citizen.Wait(1500) end end end)
...
AddEventHandler("utk_fh:outcome", function(oc, arg)
    ...
    elseif not oc then
        exports['hNotifikacije']:Notifikacije(msg, 2)
    end
end)
            ...
            while true do
                ...
                if dst < 40 then
                    if not LootCheck[name].Loot1 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley1.x, data.trolley1.y, data.trolley1.z+1, "[~r~E~w~] da uzmes novac", 0.40)
                    ...
                    if not LootCheck[name].Loot2 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley2.x, data.trolley2.y, data.trolley2.z+1, "[~r~E~w~] da uzmes novac", 0.40)
                    ...
                    if not LootCheck[name].Loot3 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley3.x, data.trolley3.y, data.trolley3.z+1, "[~r~E~w~] da uzmes novac", 0.40)
                            if dst1 < 1 and IsControlJustReleased(0, 38) then
...
AddEventHandler("utk_fh:policenotify", function(name)
    ...
    if PlayerData.job.name == "police" then
        exports['hNotifikacije']:Notifikacije('Pljacka Sporedne banke je u toku', 1)
...
AddEventHandler("utk_fh:freezeDoors", function()
    ...
    --                    if dst <= 4.0 then
    --                        if v[i].locked then
    --                            DrawText3D(v[i].txtloc[1], v[i].txtloc[2], v[i].txtloc[3], "[~r~E~w~] Om de deur te openen", 0.40)
    --                        elseif not v[i].locked then
    --                            DrawText3D(v[i].txtloc[1], v[i].txtloc[2], v[i].txtloc[3], "[~r~E~w~] Om de deur op slot te doen", 0.40)
    --                        end
...
AddEventHandler("utk_fh:reset", function(name, data)
    ...
    Check[name] = false
    exports['hNotifikacije']:Notifikacije('Vrata sefa ce se zatvoriti za 30 sekundi', 2)
    Citizen.Wait(30000)
    exports['hNotifikacije']:Notifikacije('Sef je zatvoren', 2)
...
AddEventHandler("utk_fh:startheist", function(data, name)
    ...
    --exports['progressBars']:startUI(2000, "Gebruikt Gehackte Toegangs Kaart")
    Citizen.Wait(1500)
    ...
    disableinput = false
    Citizen.Wait(2000)
    --Process(UTK.hacktime, "Hack is bezig")
    --Citizen.Wait(UTK.hacktime)
    exports['hNotifikacije']:Notifikacije('Hakovanje je uspelo', 3)
    PlaySoundFrontend(-1, "ATM_WINDOW", "HUD_FRONTEND_DEFAULT_SOUNDSET")
    TriggerServerEvent("utk_fh:toggleVault", name, false)
    startdstcheck = true
    currentname = name
    exports['hNotifikacije']:Notifikacije('Imas 2 minuta do aktiviranja sigurnosnog sistema', 3)
...
function SecondDoor(data, key)
    ...
    ClearPedTasksImmediately(ped)
    disableinput = false
    --Process(2000, "Toegang tot paneel verkrijgen")
    Citizen.Wait(2000)
    PlaySoundFrontend(-1, "ATM_WINDOW", "HUD_FRONTEND_DEFAULT_SOUNDSET")
		    ...
		    while GetGameTimer() - startedGrabbing < 37000 do
			    ...
			    if HasAnimEventFired(ped, GetHashKey("RELEASE_CASH_DESTROY")) then
				    if IsEntityVisible(grabobj) then
                        SetEntityVisible(grabobj, false, false)
                        ESX.TriggerServerCallback("utk_fh:rewardCash", function() end)
				    end
			    end
		    end
...
Citizen.CreateThread(function()
    while true do
        if not done then
            ...
            if dst < 4 then
                DrawText3D(UTK.Banks[currentname].doors.secondloc.x, UTK.Banks[currentname].doors.secondloc.y, UTK.Banks[currentname].doors.secondloc.z, "[~r~E~w~] Da hakujes panel", 0.40)
                if dst < 0.75 and IsControlJustReleased(0 , 38) then
                    ESX.TriggerServerCallback("utk_fh:checkSecond", function(result)
                        ...
                        elseif not result then
                            exports['hNotifikacije']:Notifikacije('Nemas Sigurnosnu Karticu', 3)
                        end
                    end)
                end
            end
...
Citizen.CreateThread(function()
    ...
    while ESX.GetPlayerData().job == nil do
        Citizen.Wait(500)-- waits opgehoogd
    end
            ...
            for k, v in pairs(UTK.Banks) do
                if not v.onaction then
                    ...
                    if dst < 5 and not Check[k] then
                        DrawText3D(v.doors.startloc.x, v.doors.startloc.y, v.doors.startloc.z, "[~r~E~w~] da pocnes pljacku", 0.40)
