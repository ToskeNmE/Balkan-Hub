...
local mincash = 500 -- minimum amount of cash a pile holds
local maxcash = 1000 -- maximum amount of cash a pile can hold
    ...
    if not info.locked then
        if (os.time() - cooldown) > lastrobbed then
            ...
            if copcount >= mincops then
                if method == 1 then
                    local item = yPlayer.getInventoryItem("thermal")["count"]

                    if item >= 1 then
                        yPlayer.removeInventoryItem("thermal", 1)
                        cb(true)
                        info.stage = 1
                        info.style = 1
                        info.locked = true
                    else
                        cb("Nemas termalnu bombu.")
                    end
                end
            else
                cb("Potrebno je minimum "..mincops.." policajaca u gradu.")
            end
        else
            cb(math.floor((cooldown - (os.time() - lastrobbed)) / 60)..":"..math.fmod((cooldown - (os.time() - lastrobbed)), 60).." preostalo do sledece pljacke.")
        end
    else
        cb("Banka se vec pljacka.")
    end
end)
...
ESX.RegisterServerCallback('utk_oh:rewardCash', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('utk_oh:rewardGold', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)

    xPlayer.addInventoryItem("gold_bar", 1)
end)
ESX.RegisterServerCallback('utk_oh:rewardDia', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
