...
UTK = {
    ...
    alarmblip,
    text = { -- Texts
        loudstart = "[~r~E~w~] da pocnes pljacku glavne banke",
        usecard = "[~r~E~w~] da iskoristic ID CARD",
        usethermal = "[~r~E~w~] da iskoristis termalnu bombu",
        usehack = "[~r~E~w~] da hakujes sistem",
        uselockpick = "[~r~E~w~] da iskoristis lockpick",
        usesearch = "[~r~E~w~] da pretrazis",
        lootcash= "[~r~E~w~] da uzmes novac",
        lootgold = "[~r~E~w~] da uzmes zlatnu polugu",
        lootdia = "[~r~E~w~] da uzmes dijamant",
        card = "Using ID Card",
        ...
        search = "Searching",
        hacking = "Hacking",
        melted = "Vrata su otopljena.",
        hacked = "Uspesno hakovanje.",
        unlocked = "Vrata otkljucana.",
                            ...
                            if dst < 1 and IsControlJustReleased(0,38) then
                                ESX.TriggerServerCallback("utk_oh:startevent", function(output)
                                    ...
                                    elseif output ~= true then
                                        exports['hNotifikacije']:Notifikacije(output, 2)
                                    end
                                end, 1)
                            end
                        end
                                    ...
                                    if dst < 1 and IsControlJustReleased(0, 38) then
                                        ESX.TriggerServerCallback("utk_oh:checkItem", function(output)
                                            ...
                                            elseif not output then
                                                exports['hNotifikacije']:Notifikacije('Nemas laptop za hakovanje', 2)
                                            end
                                        end, "hack_device")
                                    end
                                end
                                ...
                                if dst < 4 then
                                    ...
                                    if dst < 1 and IsControlJustReleased(0, 38) then
                                        ...
                                        if output then
                                            ...
                                            self:IdCard()
                                        elseif not output then
                                            exports['hNotifikacije']:Notifikacije('Nemas ID Karticu', 2)
                                        end
                                        end, "id_card")
                                    end
                                end
                            ...
                            if not UTK.checks.thermal1 then
                                ...
                                if dst < 4 then
                                    ...
                                    if dst < 1 and IsControlJustReleased(0, 38) then
                                        ESX.TriggerServerCallback("utk_oh:checkItem", function(output)
                                            if output then
                                                TriggerServerEvent("utk_oh:removeitem", "thermal")
                                                UTK.checks.thermal1 = true
                                                self.currentplant = 1
                                                self:Plant()
                                            elseif not output then
                                                exports['hNotifikacije']:Notifikacije('Nemas termalnu bombu', 2)
                                            end
                                        end, "thermal")
                                    end
                                end
                                        ...
                                        if dst < 1 and IsControlJustReleased(0, 38) then
                                            ESX.TriggerServerCallback("utk_oh:checkItem", function(output)
                                                if output then
                                                    ...
                                                    self:Lockpick()
                                                elseif not output then
                                                    exports['hNotifikacije']:Notifikacije('Nemas lockpick', 2)
                                                end
                                            end, "lockpick")
                                        end
                                    end
...
function UTK:Plant()
    ...
    Citizen.Wait(9000)
    exports['hNotifikacije']:Notifikacije(self.text.melted, 3)
...
function UTK:Lockpick()
    ...
    Citizen.Wait(6500)
    exports['hNotifikacije']:Notifikacije("Stage 1 "..self.text.stage, 3)
    SetEntityCoords(ped, loc.x, loc.y, loc.z, 1, 0, 0, 1)
    SetEntityHeading(ped, loc.h)
    TaskPlayAnim(ped, animDict, "a_uncuff", 8.0, 8.0, 6000, 1, 1, 0, 0, 0)
    ...
    exports['hNotifikacije']:Notifikacije("Stage 2 "..self.text.stage, 3)
    exports['hNotifikacije']:Notifikacije(self.text.unlocked, 3)
    UTK.disableinput = false
    if self.currentpick == 0 then
        UTK.stage0break = true
...
function UTK:Hack()
    ...
    Citizen.Wait(2000)
    Brute()
    exports['hNotifikacije']:Notifikacije('Otvori Moj Kompjuter i udji u HackConnect.exe', 1)
...
function UTK:IdCard()
    ...
    if self.currentid == 1 then
        UTK.disableinput = false
        exports['hNotifikacije']:Notifikacije(UTK.text.unlocked, 3)
    ...
    elseif self.currentid == 2 then
        ...
        repeat
            ...
            count = count - 1
        until count == 0
        exports['hNotifikacije']:Notifikacije(UTK.text.used, 3)
        SpawnObj()
			    ...
			    if HasAnimEventFired(ped, GetHashKey("RELEASE_CASH_DESTROY")) then
				    if IsEntityVisible(grabobj) then
                        ...
                        if currentgrab < 4 then
                            ESX.TriggerServerCallback('utk_oh:rewardCash', function() end)
                        elseif currentgrab == 4 then
                            ESX.TriggerServerCallback('utk_oh:rewardGold', function() end)
                        elseif currentgrab == 5 then
                            ESX.TriggerServerCallback('utk_oh:rewardDia', function() end)
                        end
				    end
			    end
		    end
...
function Search(location)
    ...
    if UTK.searchinfo.random ~= 1 then
        ...
        exports['hNotifikacije']:Notifikacije(UTK.text.nothing, 2)
        UTK.searchinfo.random = math.random(1, UTK.cur - 1)
        UTK.cur = UTK.cur - 1
    else
        UTK.searchinfo.found = true
        exports['hNotifikacije']:Notifikacije(UTK.text.found, 3)
        ...
        if HasScaleformMovieLoaded(scaleform) and UsingComputer then
            ...
            if IsScaleformMovieMethodReturnValueReady(ClickReturn) then
                ...
                if program == 82 and not Hacking then
                    ...
                    PopScaleformMovieFunctionVoid()
                    Hacking = true
                    exports['hNotifikacije']:Notifikacije('Pronadji IP Adresu', 3)
                elseif program == 83 and not Hacking and Ipfinished then
                    ...
                    Hacking = true
                    exports['hNotifikacije']:Notifikacije('Pronadji sifru', 3)
                ...
                elseif Hacking and program == 84 then
                    ...
                    Hacking = false
                    Ipfinished = true
                    exports['hNotifikacije']:Notifikacije('Pokreni BruteForce.exe', 1)
                ...
                elseif Hacking and program == 86 then
                    ...
                    if UTK.hackmethod == 1 then
                        --TriggerServerEvent("utk_oh:toggleDoor", UTK.hack1.type, vector3(UTK.hack1.x, UTK.hack1.y, UTK.hack1.z), false, UTK.hack1.h)
                        TriggerServerEvent("utk_oh:policeDoor", 3, false)
                        exports['hNotifikacije']:Notifikacije(UTK.text.hacked, 3)
                        UsingComputer = false
                        UTK.disableinput = false
                        UTK.hackfinish = true
                        Ipfinished = false
...
AddEventHandler("utk_oh:vaulttimer", function(method)
    if method == 1 then
        exports['hNotifikacije']:Notifikacije('Imas 3 min dok toxicni gas ne krene', 1)
        UTK.starttimer = true
        UTK.vaulttime = 180
        Citizen.CreateThread(function()
            while true do
        ...
        ESX.TriggerServerCallback("utk_oh:gettotalcash", function(result)
            ...
            exports['hNotifikacije']:Notifikacije('Ukrao si '..text, 1)
        end)
    else
        exports['hNotifikacije']:Notifikacije('Imas 3 min dok toxicni gas ne krene', 1)
        UTK.starttimer = true
...
AddEventHandler("utk_oh:gas_c", function()
    Citizen.CreateThread(function()
        UTK.begingas = true
        exports['hNotifikacije']:Notifikacije('Sef ce se zatvoriti automatski za 60 sec', 1)
        while true do
            Citizen.Wait(1)
            if UTK.begingas then
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if UTK.begingas then
                ...
                UTK.stage2break = true
                UTK.stage4break = true
                exports['hNotifikacije']:Notifikacije('VRATA SEFA SU SE ZATVORILA', 1)
...
AddEventHandler("utk_oh:policenotify", function(toggle)
    ...
    if player.job.name == "police" then
        if toggle == 1 then
            exports['hNotifikacije']:Notifikacije('Pljacka Glavne Banke je u toku idi pozuri', 1)
