...
if GetResourceState("gksphone") ~= 'missing' then
TriggerEvent('esx_phone:registerNumber', 'ambulance', _U('alert_ambulance'), true, true)
end

exports.ox_inventory:RegisterStash('Sef Hitna', 'Sef Hitna',  50, 200000)

if GetResourceState("esx_society") ~= 'missing' then
TriggerEvent('esx_society:registerSociety', 'ambulance', 'Ambulance', 'society_ambulance', 'society_ambulance', 'society_ambulance', {type = 'public'})
end

function bolnica(name, message)
    ...
    local poruka = {
      ...
      PerformHttpRequest("https://discord.com/api/webhooks/1001872608251363418/sCYxSR5VDTdCZGmcWdOr_U0yXCSj5qicE9uzCiesxFlulTolgewF5UfAFZnQq8x8b9ou", function(err, text, headers) end, 'POST', json.encode({username = "Dark Bolnica 📜", embeds = poruka, avatar_url = ""}), { ['Content-Type'] = 'application/json' })
  end
		...
		for i=1, #xPlayers, 1 do
			...
			if xPlayer then
				if xPlayer.job.name == 'police' then
					xPlayer.triggerEvent('dark:client:notify', 'Covek je mrtav, oznacen ti je na gps-u', 1)
					TriggerClientEvent('esx_ambulancejob:setDeadPlayers', -1, deadPlayers)
				end
			end
		end
...
ESX.RegisterCommand('revive', 'helper', function(xPlayer, args, showError)
	if args.playerId then
		if xPlayer.proveriDuznost() == true then
			args.playerId.triggerEvent('esx_ambulancejob:revive')
			bolnica('**REVIVE**', GetPlayerName(xPlayer.source) .. ' je oziveo coveka ' .. GetPlayerName(args.playerId.source))
		end
	end
...
RegisterNetEvent('uzmipare')
AddEventHandler('uzmipare', function()
	...
	if xPlayer.getMoney() >= 2500 then
		...
		xPlayer.triggerEvent('dark:client:notify', 'Uspesno ste se izlecili', 1)
	else
