...
function StartDistressSignal()
	CreateThread(function()
		...
		while timer > 0 and isDead do
			...
			AddTextComponentSubstringPlayerName('Pritisni ~g~G~w~ da pozoves bolnicara')
			EndTextCommandDisplayText(0.175, 0.805)

			if IsControlJustReleased(0, 47) then
				TriggerEvent('dark:npc:pozovi')
...
--[[RegisterNetEvent('esx_ambulancejob:oznaciGPS')
AddEventHandler('esx_ambulancejob:oznaciGPS', function()
	local playerPed = PlayerPedId()
	local coords = GetEntityCoords(playerPed)

	for i=1, #playerPed, 1 do
...
local coords = { x = 312.8276, y = -592.374, z = 43.284, h = 312.09 }
...
Citizen.CreateThread(function()
    while true do
        ...
        if distance < 3 then
			spavanje = false
		    exports['hCore']:Draw3DText(312.8259, -592.373, 43.284, "[E] Da se izlecite | 2500$")
            if IsControlJustReleased(0, 38) then
				TriggerServerEvent('uzmipare')
            end
        end
