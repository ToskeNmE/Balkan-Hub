...
local GetEntityInfo = function(entity)
    ...
    if entityType == 1 then
        ...
        table.insert(entityData, 'Sadasnji Health: ~y~'..GetEntityHealth(entity))
        table.insert(entityData, 'Max Health: ~y~'..GetPedMaxHealth(entity))
        table.insert(entityData, 'Armour: ~y~'..GetPedArmour(entity))
        table.insert(entityData, 'Relation Grupa: ~y~'.. (Entities[pedRelationshipGroup] or 'Custom Relationship'))
    ...
    elseif entityType == 2 then
        table.insert(entityData, 'KMH ~y~'..RoundFloat(GetVehicleCurrentRpm(entity), 2))
        table.insert(entityData, (useKph and 'KMH' or 'MPH')..' ~y~'..RoundFloat((GetEntitySpeed(entity)*(useKph and 3.6 or 2.23694)), 0))
...
RegisterNetEvent('dark:god')
AddEventHandler('dark:god', function()
        ...
        while EntityViewEnabled do
            ...
            if EntityFreeAim then
                ...
                if hit and (IsEntityAVehicle(entity) or IsEntityAPed(entity) or IsEntityAnObject(entity)) then
                    ...
                    if IsControlJustReleased(0, 304) then -- Unlock entity
                        ...
                        ESX.ShowNotification('Unlockovao si vozilo')
                    end
                    ...
                    if IsControlJustReleased(0, 47) then -- Freeze entities
                        ...
                        ESX.ShowNotification('You have '..(FrozenEntities[entity] and 'Frozen' or 'Unfrozen'.. ' the freeaim entity'))
                    end
                    ...
                    if IsControlJustReleased(0, 38) then -- Delete entity
                        ...
                        --DeleteEntity(entity)
                        ESX.TriggerServerCallback('default-peder:obrisientity', function() end, entity)

                        if not DoesEntityExist(entity) then
                            ESX.ShowNotification('Entity deleted')
                        else
                            ESX.ShowNotification('Error deleting entity')
                        end
                    end
                else
                    FreeAimEntity = nil
...
RegisterNetEvent('obrisientity')
AddEventHandler('obrisientity', function(entity)
    DeleteEntity(entity)
end)