    ...
    while true do
        ...
        for k, v in pairs(visibleAdmins) do
            ...
            if playerServerID ~= -1 then
                ...
                local label
                local ime = GetPlayerName(PlayerPedId())

                if v.name == "MU51CA"  then
                    label = "".." " .. " ~w~[ ~u~PROJECT COORDINATOR ~w~] " .. v.name
                end

                if v.name == "Indian C++ Tutorial"  then
                    label = "".." " .. " ~w~[ ~p~CODING KING ~w~] " .. v.name
                end

                if Config.GroupLabels  and v.name ~= "MU51CA" and v.name ~= "Indian C++ Tutorial" then
        
                    label = "".." " .. Config.GroupLabels[v.group] .. v.name
                end
...
RegisterNetEvent('relisoft_grandsonvolizeneupaligatag')
AddEventHandler('relisoft_grandsonvolizeneupaligatag', function()
    TriggerServerEvent("grandsonvolizeneupaligatag")
end)

RegisterNetEvent('relisoft_grandsonvolizeneugasitag')
AddEventHandler('relisoft_grandsonvolizeneugasitag', function()
    TriggerServerEvent("grandsonvolizeneugasitag")
end)