...
function Core.SavePlayer(xPlayer, cb)
	MySQL.prepare('UPDATE `users` SET `accounts` = ?, `job` = ?, `job_grade` = ?, `group` = ?, `position` = ?, `inventory` = ?, `loadout` = ? WHERE `identifier` = ?', {
		...
		if affectedRows == 1 then
			print(('[^2INFO^7] Sacuvao sam igraca ^5"%s^7"'):format(xPlayer.name))
		end
		if cb then cb() end
	end)
end
...
function Core.SavePlayers(cb)
	...
	if count > 0 then
		...
		function(results)
			if results then
				if type(cb) == 'function' then cb() else print(('[^2INFO^7] Sacuvao sam %s %s za %s ms'):format(count, count > 1 and 'players' or 'player', (os.time() - time) / 1000000)) end
			end
		end)
	end
end
